...
from .utils import scipy_smooth
...
def demo_model(preset, **kwargs):
    ...
    * 'marmousi3d-tti': Loads the 2D Marmousi data set from the given
    ...
    if preset.lower() in ['constant-poroelastic']:
        ...
        # Matrix Properties
        G = np.ones(shape, dtype=dtype)*kwargs.pop('G', 15e09)        # Shear Modulus, Pa = kg / (m * s**2)
        phi = np.ones(shape, dtype=dtype)*kwargs.pop('phi', 0.16)       # Porosity, %
        k = np.ones(shape, dtype=dtype)*kwargs.pop('k', 9.8692327E-16)         # Permeability, m**2
        K_dr = np.ones(shape, dtype=dtype)*kwargs.pop('K_dr', 6e09)  # Drained bulk modulus, Pa = kg / (m * s**2)
        T = np.ones(shape, dtype=dtype)*kwargs.pop('T', 10.0)           # Tortuosity, -

        # Solid Properties
        rho_s = np.ones(shape, dtype=dtype)*kwargs.pop('rho_s', 2500.)  # Solid grain density, kg/m**3
        ...
        mu_f = np.ones(shape, dtype=dtype)*kwargs.pop('mu_f', 0.0001)   # Fluid viscosity, Pa*s = (kg / (m * s**2)) * s
...
class ModelPoroelastic(GenericModel):
    ...
    def __init__(self, origin, spacing, shape, space_order, rho_s, rho_f,
                 phi, k, mu_f, K_dr, K_s, K_f, G, T, nbpml=20, dtype=np.float64):
        ...
        # Bulk Density
        self.rho_b = Function(name="rho_b", grid=self.grid, space_order=space_order)
        initialize_function(self.rho_b, self.rho_s.data  * (1.0-self.phi.data) + self.rho_f.data * self.phi.data, self.nbpml)
        self._physical_parameters += ('rho_b',)
    ...
    def critical_dt(self):
        ...
        #
        # The CFL condtion is then given by
        return self.dtype(np.min( np.min(self.spacing) / (self.Vp.data**2 - self.Vs.data**2)**0.5) / 10.0 )
